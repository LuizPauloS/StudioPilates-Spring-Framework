...
public class PagamentoController {
	...
	public String adicionaPagamento(@Validated Pagamento pagamento, Errors errors, RedirectAttributes attributes) {
		if(errors.hasErrors()){
			return NEW_PAGAMENTO;
		}
		if (pagamento.getId() != null) {
			this.pagamentoService.cadastrarPagamento(pagamento);
			attributes.addFlashAttribute("mensagem", "Pagamento do paciente " + pagamento.getPaciente().getNome() + " alterado com sucesso!");
		} else {
			pagamento.setDataCadastro(new Date());
			this.pagamentoService.cadastrarPagamento(pagamento);
			attributes.addFlashAttribute("mensagem", "Pagamento do paciente " + pagamento.getPaciente().getNome() + " adicionado com sucesso!");
		}
	...
	public ModelAndView pesquisarPagamentos(@ModelAttribute("pagamentoFilter")PagamentoFilter pagamentoFilter){
